...
import numpy as np  # <-- ensure numpy is imported once at top
...
def process_all_tissue_mask_filter(
    ...
    for sample_dir in tqdm(sample_dirs, desc="Processing samples"):
        ...
        try:
            ...
            # Count initial cells (cast to native int)
            initial_count = int(count_cells(seg_mask))
            ...
            if has_tissue_mask and initial_count > 0:
                ...
                # Calculate tissue area (number of pixels) and cast to int
                tissue_area_pixels = int(np.sum(tissue_mask_eroded > 0))
                ...
                # Count filtered cells (cast to int)
                filtered_count = int(count_cells(filtered_mask))
                cells_removed = int(initial_count - filtered_count)
                percent_retained = float((filtered_count / initial_count * 100) if initial_count > 0 else 0.0)

                # Calculate average density (cells per pixel) -> float
                average_density = float(filtered_count / tissue_area_pixels) if tissue_area_pixels > 0 else 0.0
                ...
                filtered_count = int(initial_count)
                cells_removed = int(0)
                percent_retained = float(100.0 if initial_count > 0 else 0.0)
                tissue_area_pixels = int(0)
            ...
            # Collect results (ensure native types)
            result = {
                'sample': sample_name,
                'status': 'success',
                'error': None,
                ...
                'erosion_downsample_factor': erosion_downsample_factor,
                'min_overlap_ratio': min_overlap_ratio,
                'initial_cells': int(initial_count),
                'tissue_filtered_cells': int(filtered_count),
                'cells_removed_by_tissue': int(cells_removed),
                'percent_retained': float(percent_retained),
                'tissue_area_pixels': int(tissue_area_pixels),
                'average_density_per_pixel': float(average_density),
                'average_density_per_100x100': float(average_density * 10000) if tissue_area_pixels > 0 else 0.0,
    ...
    print(f"Samples with tissue masks: {int(df['has_tissue_mask'].fillna(False).sum())}")

    # Statistics for samples with tissue masks
    tissue_filtered = df[df['has_tissue_mask'] == True]
    if len(tissue_filtered) > 0:
        total_initial = int(tissue_filtered['initial_cells'].sum())
        total_filtered = int(tissue_filtered['tissue_filtered_cells'].sum())
        total_removed = int(tissue_filtered['cells_removed_by_tissue'].sum())
        total_tissue_area = int(tissue_filtered['tissue_area_pixels'].sum())
        ...
        avg_density_per_sample = float(tissue_filtered['average_density_per_100x100'].mean())
        std_density_per_sample = float(tissue_filtered['average_density_per_100x100'].std())
        print(f"\nAverage density across samples:")
        print(f"  Mean: {avg_density_per_sample:.2f} ± {std_density_per_sample:.2f} cells per 100x100 area")
        print(f"  Range: {float(tissue_filtered['average_density_per_100x100'].min()):.2f} - {float(tissue_filtered['average_density_per_100x100'].max()):.2f}")
